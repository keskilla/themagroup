...
<div class="col-md-10" style="float: right">
  ...
  <div class="alert alert-info small" role="alert">
    ...
    <li> Il vous suffit de choisir le nom de l'actualité.</li>
  </div>
  ...
  <form method="post" action="index.php?action=actuDelete" style="border:solid 1px #0000001f; box-shadow: 1px 1px 12px #55555585;border-radius:5px;">
    <div class="p-2">
      <div class="form-group">
        <label for="exampleeFormControlSelect1">Choisir le nom de l'actualité</label>
        <select class="form-control" id="exampleeFormControlSelect1" name="selectNameVignette">
          <?php while ($data = $post->fetch()){?> 
          <?=' <option>'.htmlspecialchars($data['titre_vignette']).'</option> '?>
          <?php } ?>
        </select>
      </div>
